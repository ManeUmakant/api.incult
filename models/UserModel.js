...
const QueryExecutor = require('../util/QueryExecutor');
 ...
 UserModel.findUserById = async (userId) =>{
    try {
        const result = await QueryExecutor.execute(`select * from users where user_id=${userId}`);
        return result;
    }catch(e) {
        throw e;
    }   
...
UserModel.createUser = async (obj) =>{
    ...
    const query = `INSERT INTO users (user_name, user_email, user_phone) 
    VALUES (${obj.userName}, ${obj.userEmail}, ${obj.number})`;
    const result = await QueryExecutor.execute(query);
    return result;
}

UserModel.updateUserAvatar = async (obj) =>{
    let query = `UPDATE users SET user_avatar = '${obj.avatarPath}'
     WHERE user_id = ${obj.userId}`;
    await QueryExecutor.execute(query);
}

UserModel.updateUserProfile =   async (obj) => {
     let query = `UPDATE users SET user_name = '${obj.userName}', 
